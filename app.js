...
/* ---- utils ---- */
function save(k, v){ try{ localStorage.setItem(k, JSON.stringify(v)); }catch(e){ console.error("save",k,e); toast("Falha ao salvar dados.", "error"); } }
function load(k, d){ try { const raw = localStorage.getItem(k); return raw ? JSON.parse(raw) : d; } catch(e){ console.warn("load fallback",k,e); return d } }
function uid(){ return Math.random().toString(36).slice(2,9) + Date.now().toString(36).slice(-4); }
function brMoney(n){ const x = Number(n)||0; try{ return x.toLocaleString('pt-BR',{style:'currency', currency:'BRL'}); } catch{ return `R$ ${x.toFixed(2)}`; } }
...
function toast(msg, type=""){ 
  ...
  el.innerHTML = `<div class="t-title">${type==='error'?'Erro':type==='success'?'Sucesso':type==='warn'?'Aviso':'Info'}</div>
...
navButtons.vender.addEventListener("click", ()=>show("vender"));
navButtons.produtos.addEventListener("click", ()=>show("produtos"));
navButtons.clientes.addEventListener("click", ()=>show("clientes"));
navButtons.backup.addEventListener("click", ()=>show("backup"));
...
$("#btnWhatsapp").addEventListener("click", ()=>{
  ...
  window.open("https://wa.me/5565984491018?text=" + encodeURIComponent(textoEntrega), "_blank");
});
...
function buildOrder(){
  ...
  const valorRaw = saleValor.value?.toString().replace(",", ".") || "";
  const valor = parseFloat(valorRaw || (prod?.valor??0)) || 0;
...
function renderPedidos(){
  const el = $("#ordersList");
  if(!Array.isArray(state.pedidos) || state.pedidos.length===0){ el.innerHTML = `<div class="muted">Nenhum pedido.</div>`; return; }
  ...
  $$(".openOrder").forEach(b=>b.addEventListener("click", ()=>{
    const o = state.pedidos.find(x=>x.id===b.dataset.id);
    if(!o) return;
    window.open(makeViewerUrl(o), "_blank");
  }));

  $$(".delOrder").forEach(b=>b.addEventListener("click", ()=>{
...
$("#btnAddProduto").addEventListener("click", ()=>{
  ...
  const valorRaw = $("#prodValor").value?.toString().replace(",", ".") || "";
...
function renderProdutos(){
  ...
  if(!Array.isArray(state.produtos) || state.produtos.length===0){ el.innerHTML = `<div class="muted">Nenhum produto.</div>`; return; }
  ...
  $$(".editProd").forEach(b=>b.addEventListener("click", ()=>editProduto(b.dataset.id)));
  $$(".delProd").forEach(b=>b.addEventListener("click", ()=>{
    const id = b.dataset.id;
...
function editProduto(id){
  ...
  const valor = parseFloat((valorStr||"").toString().replace(",", ".")) || p.valor;
...
function renderClientes(){
  ...
  if(!Array.isArray(state.clientes) || state.clientes.length===0){ el.innerHTML = `<div class="muted">Nenhum cliente.</div>`; return; }
  ...
  $$(".editCli").forEach(b=>b.addEventListener("click", ()=>editCliente(b.dataset.id)));
  $$(".delCli").forEach(b=>b.addEventListener("click", ()=>{
    const id = b.dataset.id;
...
function parseCSV(text){
  ...
  // Detecta separador por heurística simples
  let sep = ",";
  const first = lines[0];
  const commaCount = (first.match(/,/g)||[]).length;
  const semiCount = (first.match(/;/g)||[]).length;
  if(semiCount>commaCount) sep = ";";
  // Parser com suporte a aspas
